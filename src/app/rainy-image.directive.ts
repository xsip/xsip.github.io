...
@Directive({
  selector: '[appRainyImage]',
  standalone: true
})
export class RainyImageDirective implements OnInit {
  platformId = inject(PLATFORM_ID);
  canvas!: HTMLCanvasElement;
  particlesArray: Particle[] = [];
  numberOfParticles = 12000;
  detail = 1;
  ctx: any;
  grid: any[] = []
  constructor(
  ...
  ngOnInit() {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      image.addEventListener('load', () => {
        ...
        this.canvas.className =' fixed z-[-1] top-0 left-0 w-full h-full';
        ...
        for (let y = 0; y < this.canvas.height; y += detail) {
          let row = [];
          for (let x = 0; x < this.canvas.width; x += detail) {
            const red = pixels.data[(y * 4 * pixels.width) + (x * 4)]
            const green = pixels.data[(y * 4 * pixels.width) + (x * 4 + 1)]
            const blue = pixels.data[(y * 4 * pixels.width) + (x * 4 + 2)]
            const color = 'rgb(' + red + ',' + green + ',' + blue + ')';
            const brightness = this.calculateBrightness(red, green, blue) / 100;
  ...
  calculateBrightness(red: number, green: number, blue: number) {
    return Math.sqrt(
      (red * red) * 0.299 +
      (green * green) * 0.587 +
      (blue * blue) * 0.114
...
class Particle {
  ...
  constructor(private canvas: HTMLCanvasElement, private ctx: CanvasRenderingContext2D, private detail: number) {
  ...
  update(grid: any[] = [] ) {
    this.speed = grid[Math.floor(this.y / this.detail)][Math.floor(this.x / this.detail)];
